...
#
# Usage:
#   OIDC_CLIENT_KEY=<secret> OIDC_ADMIN_USERS="<account> ..." \
#     lib/apply-oidc-realm.sh proxmox/oidc-<realm>.env root@<node>
#
# The client secret and the admin account list are run-time inputs, not
# declared file content. The secret's canonical store is the IdP itself;
# PVE persists it in pmxcfs (/etc/pve/domains.cfg, plaintext,
# root-readable) — a known PVE property, not a choice this script makes.
set -euo pipefail
...
ENV_FILE="${1:?usage: OIDC_CLIENT_KEY=<secret> OIDC_ADMIN_USERS=\"<account> ...\" apply-oidc-realm.sh proxmox/oidc-<realm>.env root@<node>}"
NODE_SSH="${2:?second arg required: root@<node>}"
...
echo "== admin ACLs =="
for email in ${OIDC_ADMIN_USERS:-}; do
  userid="${email}@${REALM}"
  ssh "${NODE_SSH}" "pveum user add '${userid}' --comment 'OIDC operator (applied by infra-proxmox lib/apply-oidc-realm.sh)' 2>/dev/null || true"
